...
- **抽象クラス（abstract class）**: 一部のメソッドが未実装（抽象メソッド）で、直接`new`してインスタンス化できないクラス
- **抽象メソッド**: 中身（`{}`の処理）を持たず、シグネチャだけを宣言したメソッド。継承した子クラスに実装を強制する

### 誤解しやすいポイント：`abstract`が付けられるのは2箇所だけ

「抽象クラス＝中身を持たないフィールドやメソッドの集まり」という理解は誤り。`abstract`という単語が付けられるのは、**①クラス自体（`abstract class`）**と、**②明示的に`abstract`と書かれたメソッド**の2箇所だけ。**フィールドに`abstract`を付けるという概念はそもそも存在しない**（フィールドは常に普通の値を持つ）。`abstract`が付いていない普通のメソッド（`printName()`など）も、他のクラスと同じように中身を持てる。抽象クラスは「一部だけ`abstract`（中身なし）が混在してもよいクラス」であって、「全部が中身なし」ではない。
...
### 「抽象クラス＝中身を持たないフィールドやメソッドのクラス」という誤解

**何が起きたか**: 復習チェックポイントで、抽象クラスを「中身を持たないフィールドやメソッドのクラス、中身は継承先に委ねる」と説明した。`Employee`の`printName()`（中身あり）を思い出してもらうことで、`abstract`が付いていない普通のメソッドは中身を持てることに気づけた。

**なぜ**: 「抽象」という言葉のイメージから、クラス全体が「未完成・中身なし」だと連想してしまっていた。実際には`abstract`が付けられるのはクラス自体とメソッドだけで、フィールドや`abstract`なしのメソッドは普通のクラスと同じように中身を持てる。
