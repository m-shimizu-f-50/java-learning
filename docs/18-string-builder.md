...
- **イミュータブル（不変）**: 一度作られたら中身を変更できない性質。変更したように見える操作は、実は新しいオブジェクトを作って返しているだけ
- **文字列プール**: 同じ内容の文字列リテラル（`""`で書いたもの）を使い回すための、JVM内の特別な領域
- **StringBuilder**: 中身を変更できる（ミュータブルな）文字列操作用クラス。ループ内での文字列連結に向く
