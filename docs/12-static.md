...
**何が起きたか**: `review/22-static`の復習で、`static`なしの`final String orderId`（インスタンスごとに`"A001"`/`"A002"`と別の値を持つ）を提示し、「`o1.orderId`と`o2.orderId`は別々の値を持てる」と正しく答えた直後、「`static`と`final`は何を決めているか」を一言でまとめてもらうと、「`final`は定数の値」という説明に戻ってしまった。「同じ`final`のフィールドで、なぜ`o1`と`o2`が違う値を持てるのか」と矛盾を突きつけると「持てない」と回答し、目の前の具体例と自分の説明が矛盾していることに気づけなかった。

**なぜ**: `static final PI`のような「共有される定数」の用法が強く印象に残っており、「`final`＝みんなが同じ1つの値を持つもの」という理解に上書きされていた。実際は「みんなで同じ値を共有するか」を決めるのは`static`の役割であり、`final`が決めるのは「そのインスタンスのそのフィールドが、一度セットされたら変更できないか」という1点だけで、「共有」とは無関係。

**教訓**: `static`と`final`の混同は、①「`static`＝変わらない値」（`12-static`本編で発生）と②「`final`＝共有される値」（今回発生）の**両方向**で起こりうる。`static final`という頻出パターンが2つの概念を1つに融合させてしまいやすいため、都度「これは共有の話か、変更可否の話か」を分けて考える。矛盾する具体例（`static`なしの`final`フィールドがインスタンスごとに別の値を持つ）を実際に提示されると、自分の説明との矛盾に気づきやすい。
